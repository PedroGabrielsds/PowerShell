...
STF-Remove-ADComputers - Remove máquinas do AD 
...
begin {
  ...
  $global:configLogDir = '\\arquivos\BDS\ESTACOES\LOGS\AD\STF-Remove-ADComputers\'
  $global:configLogName = "STF-Remove-ADComputers($DataLog).log"
  ...
  # Caminho para o arquivo contendo os nomes das máquinas
  $arquivoMaquinas = "$mainScriptRoot\maquinas_a_remover.txt"
  ...
  [String]$FaseDoScript = 'Modulo AD'

  Try {
    #Importando Modulo AD
    Import-Module ActiveDirectory 
    Write-Log -Message " Módulo do ActiveDirectory carregado com sucesso"
  } Catch {
    Write-Log -Message " Erro ao carregar o Módulo do ActiveDirectory" - Severity 3
    exit
  }


  [String]$FaseDoScript = 'Arquivo TXT'
  # Verifica se o arquivo existe
  if (-Not (Test-Path -Path $arquivoMaquinas)) {
      Write-Log -Message "O arquivo [$arquivoMaquinas] não foi encontrado." -Severity 3
      exit
  }

  Try {
    # Lê os nomes das máquinas do arquivo
    $maquinas = Get-Content -Path $arquivoMaquinas
      Write-Log -Message "O arquivo [$arquivoMaquinas] lido com sucesso."
  } Catch {
      Write-Log -Message "Não foi possível ler o arquivo [$arquivoMaquinas]" -Severity 3
      exit
  }

  # Verifica se o arquivo não está vazio
  if ($maquinas.Count -eq 0) {
      Write-Log -Message "O arquivo [$arquivoMaquinas] está vazio." -Severity 3
      exit
  } else {
      Write-Log -Message "Total de Computadores a serem removidos [$($maquinas.Count)]"
  }

  [String]$FaseDoScript = 'Principal'
  # Para cada máquina, tenta removê-la do Active Directory
  foreach ($maquina in $maquinas) {
      # Remove espaços em branco extras da linha
      $maquina = $maquina.Trim()

      # Verifica se o nome da máquina está vazio
      if (-not $maquina) {
          Write-Log -Message "Uma linha vazia foi encontrada no arquivo e será ignorada." -Severity 2
          continue
      }

      try {
        # Tenta localizar o objeto no AD
        $computador = Get-ADComputer -Filter "Name -eq '$maquina'" -ErrorAction SilentlyContinue
      } Catch {
      }

      if ($null -eq $computador) {
          Write-Log -Message "Computador [$maquina] não foi encontrado no Active Directory" -Severity 2
          continue
      }

      try {
          # Remove o computador do AD
          Remove-ADComputer -Identity $computador.DistinguishedName -Confirm:$false
          Write-Log -Message "O computador [$maquina] foi removido com sucesso do Active Directory" -Severity 0
      } catch {
          Write-Log -Message "Erro ao remover o computador [$maquina]: $_" -Severity 3
      }
  }
