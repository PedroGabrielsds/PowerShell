...
Try {
    ...
    [string]$appVendor = 'RPost'                        # Nome do Fabricante do Aplicativo (Ex.: Adobe)
    [string]$appName = 'RMail'					        # Nome do Aplicativo (Ex.: Acrobat Reader)
    [string]$appVersion = '12.0.3'                        # Versão do Aplicativo (Ex.: 1.0)
    [string]$appArch = 'X86-X64'                  	    # Arquitetura (Valores: X86, X64, X86-X64)
    [string]$appLang = 'EN'						        # Idioma (Valores: PT-BR, EN)
    [string]$appRevision = '01'					        # Número da Revisão de Pacote do Aplicativo
    [string]$appScriptVersion = '1.0.0'			        # Versão do Script 
    [string]$appScriptDate = '16/02/2024'		        # Data da criação do Pacote
    [string]$appScriptAuthor = 'Ricardo Macedo'          # Nome do Autor do Pacote
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'outlook' -Silent
        ...
        {
            Execute-MSI -Action 'Install' -Path "$dirFiles\rmail-for-outlook-desktop-32-bit.msi" -Parameters " /qn"
        }
        Else
        {
            Execute-MSI -Action 'Install' -Path "$dirFiles\rmail-for-outlook-desktop-64-bit.msi" -Parameters " /qn"
        }
	...
	#  ▼  . . . . . . . . . . . . . . . . . . . . . . . . . . .  ▼  
        ...
        If($envOfficeArchitecture.Bitness -eq "x86")
        {
            Execute-MSI -Action 'Uninstall' -Path "$dirFiles\rmail-for-outlook-desktop-32-bit.msi" -Parameters " /qn"
        }
        Else
        {
            Execute-MSI -Action 'Uninstall' -Path "$dirFiles\rmail-for-outlook-desktop-64-bit.msi" -Parameters " /qn"
        }
