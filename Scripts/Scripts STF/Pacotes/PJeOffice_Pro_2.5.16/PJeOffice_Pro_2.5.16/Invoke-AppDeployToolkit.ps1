...
#  ▼  . . . . . . . . . . . . . . . . . . . . . . . . . . .  ▼  
         ## <COLOQUE AQUI AS TAREFAS DE PRÉ-INSTALAÇÃO>
    
    ## Fecha automaticamente o PJe
    Show-ADTInstallationWelcome -CloseProcesses 'pje', 'pjeOffice', 'pjeoffice-pro'
    
    ## Mostra a Mensagem de Progresso (com a mensagem padrão)
    Show-ADTInstallationProgress
    
    #Desinstala versões antigas do PJeOffice
    # Navegador PJe - 1.0
    if (Test-Path "$envHOMEDRIVE\NavegadorPJe\unins000.exe") {
        Start-ADTProcess -FilePath "$envHOMEDRIVE\NavegadorPJe\unins000.exe" -ArgumentList '/NORESTART /SP- /VERYSILENT' -WindowStyle 'Hidden'
    }
    
    # PJE Office Build 1.0.9 - 1.0.18
    if (Test-Path "$envProgramFilesX86\pje-office-build\unins000.exe") {
        Start-ADTProcess -FilePath "$envProgramFilesX86\pje-office-build\unins000.exe" -ArgumentList '/NORESTART /SP- /VERYSILENT' -WindowStyle 'Hidden'
    }
    
    # PJE Office - 1.0.19 - 1.0.28
    if (Test-Path "$envProgramFilesX86\pje-office\unins000.exe") {
        Start-ADTProcess -FilePath "$envProgramFilesX86\pje-office\unins000.exe" -ArgumentList '/NORESTART /SP- /VERYSILENT' -WindowStyle 'Hidden'
    }
    
    # PJE Portable - 1.13.10
    If (Test-Path "$envHOMEDRIVE\PJe Portable\unins000.exe"){ 
        Start-ADTProcess -FilePath "$envHOMEDRIVE\PJe Portable\unins000.exe" -ArgumentList "/SP- /VERYSILENT"
    }
    
    # Remove a pasta PJE Portable caso ainda exista depois da desinstalação.
    If (Test-Path "$envHOMEDRIVE\PJe Portable"){ 
        Remove-ADTFolder -Path "$envHOMEDRIVE\PJe Portable" 
    }

    # Remove o ícone PJE Portable da pasta C:\Users\Public\Desktop caso exista.
    If (Test-Path "$envCommonDesktop\PJe Portable.lnk"){ 
        Remove-ADTFile -Path "$envCommonDesktop\PJe Portable.lnk" 
    }

    # Remove os atalhos da inicialização
    # PJE Office Build 1.0.9 - 1.0.18
    if (Test-Path "$envCommonStartUp\pje-office-build.lnk") {
        Remove-ADTFile -Path "$envCommonStartUp\pje-office-build.lnk"
    }
    # PJE Office - 1.0.19 - 1.0.28
    if (Test-Path "$envCommonStartUp\pje-office.lnk") {
        Remove-ADTFile -Path "$envCommonStartUp\pje-office.lnk"
    }
    # PJE Office - 2.x.x
    if (Test-Path "$envCommonStartUp\PJeOffice Pro.lnk") {
        Remove-ADTFile -Path "$envCommonStartUp\PJeOffice Pro.lnk"
    }
...
#  ▼  . . . . . . . . . . . . . . . . . . . . . . . . . . .  ▼  
    ...
    Start-ADTProcess -FilePath "$($adtSession.DirFiles)\pjeoffice-pro-v2.5.16u-windows_x64.exe" -ArgumentList "/ALLUSERS /LOG=`"$envWinDir\Logs\Software\$appName_$appVersion_Install.log`" /NORESTART /LOGCLOSEAPPLICATIONS  /SP- /VERYSILENT" -WindowStyle 'Hidden'
...
#  ▼  . . . . . . . . . . . . . . . . . . . . . . . . . . .  ▼  
    ...
    # Arquivo com as propriedade do PJeOffice Pro
    $ArquivoPropriedadesPJe = "$envProgramFiles\PJeOffice Pro\pjeoffice-update.properties"
    if (Test-Path $ArquivoPropriedadesPJe) {
        
        # Ler o conteúdo do arquivo
        $conteudo = Get-Content -Path $ArquivoPropriedadesPJe
        
        # Procurar pela linha que contém a propriedade update.url e alterá-la para 'disabled'
        $conteudo = $conteudo -replace '^update\.url=.*$', 'update.url=disabled'
        
        # Escrever o conteúdo atualizado de volta no arquivo
        Set-Content -Path $ArquivoPropriedadesPJe -Value $conteudo
        Write-ADTLogEntry -Message "Atualização Automática desabilitada."
    }
    
    # Troca a imagem de inicialização por uma totalmente TRANSPARENTE, de forma que não aparecerá nada pro usuário.
    if (Test-Path "$envProgramFiles\PJeOffice Pro\pje-icon-splash.gif") {
        Copy-ADTFile -Path "$envProgramFiles\PJeOffice Pro\pje-icon-splash.gif" -Destination "$envProgramFiles\PJeOffice Pro\pje-icon-splash-ORIGINAL.gif"
        Remove-ADTFile -Path "$envProgramFiles\PJeOffice Pro\pje-icon-splash.gif"
        Copy-ADTFile -Path "$($adtSession.DirFiles)\pje-icon-splash-transparente.gif" -Destination "$envProgramFiles\PJeOffice Pro\pje-icon-splash.gif"
    }

    #Cria o Atalho para o PJe CNJ
    Copy-ADTFile -Path "$($adtSession.DirFiles)\pjeOffice.ico" -Destination "$envProgramFiles\PJeOffice Pro\pjeOffice.ico"
    New-ADTShortcut -Path "$envCommonDesktop\PJe CNJ.lnk" -TargetPath "https://www.cnj.jus.br/pjecnj/login.seam" -IconLocation "$envProgramFiles\PJeOffice Pro\pjeOffice.ico" -Description 'PJe CNJ'
...
#  ▼  . . . . . . . . . . . . . . . . . . . . . . . . . . .  ▼  
    ...
    ## Fecha automaticamente o PJe
    Show-ADTInstallationWelcome -CloseProcesses 'pjeoffice-pro'
...
#  ▼  . . . . . . . . . . . . . . . . . . . . . . . . . . .  ▼  
    ...
    Start-ADTProcess -FilePath "$envProgramFiles\PJeOffice Pro\unins000.exe" -ArgumentList "/NORESTART /SP- /VERYSILENT" -WindowStyle 'Hidden'
...
#  ▼  . . . . . . . . . . . . . . . . . . . . . . . . . . .  ▼ 
    ...
    # Remove a pasta PJE Office Pro caso ainda exista depois da desinstalação.
    If (Test-Path "$envProgramFiles\PJeOffice Pro"){ 
        Remove-ADTFolder -Path "$envProgramFiles\PJeOffice Pro"
    }

    if (Test-Path "$envCommonDesktop\PJe CNJ.lnk") {
        Remove-ADTFile -Path "$envCommonDesktop\PJe CNJ.lnk"
    }
